...
def _normalize_input(self, x: torch.Tensor) -> torch.Tensor:
    # Детерминированная нормализация: [0, 1] → [-1, 1]
    # Всегда применяется, независимо от входного диапазона
    x = x * 2.0 - 1.0
    return x
```

**Важно:** Модель ожидает вход в диапазоне `[-1, 1]`. Нормализация выполняется автоматически и детерминированно (без условных проверок для эффективности).
